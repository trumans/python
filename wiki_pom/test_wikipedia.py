...
class TestMainPage(WikipediaCommon):
	...
	def test_mainpage_article_search(self):
		main = pages.MainPage(self.driver)
		main.open_main_page()
		main.enter_header_search_term("Disneyland")
		main.submit_header_search()
...
class TestArticlePage(WikipediaCommon):
	...
	# Parameters:
	#   search_term: text to search for an article. 
	#     assumes search opens an article and not a disambiguration page
	#   expected_value: list of (label, value) tuples where 
	#     label is contained in the left side of a row in info box
	#     value is contained in value on the right side.
	#       if value is None then label is not expected in info box
	def infobox_test(self, search_term, expected_values):
		main = pages.MainPage(self.driver)
		main.open_main_page()
		main.enter_header_search_term(search_term)
		main.submit_header_search()
